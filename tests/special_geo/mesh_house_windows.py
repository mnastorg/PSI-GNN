...
#########################################################################################################
    ...
    # LEFT WINDOW 
    gmsh.model.geo.addPoint(-0.7, -0.2, 0, size_boundary, 14)
    gmsh.model.geo.addPoint(-0.3, -0.2, 0, size_boundary, 15)
    gmsh.model.geo.addPoint(-0.3, 0.2, 0, size_boundary, 16)
    gmsh.model.geo.addPoint(-0.7, 0.2, 0, size_boundary, 17)
    gmsh.model.geo.addLine(14, 15, 14)
    gmsh.model.geo.addLine(15, 16, 15)
    gmsh.model.geo.addLine(16, 17, 16)
    gmsh.model.geo.addLine(17, 14, 17)
    gmsh.model.geo.addCurveLoop([14, 15, 16, 17], 2)

    # RIGHT WINDOW 
    gmsh.model.geo.addPoint(0.2, -0.1, 0, size_boundary, 18)
    gmsh.model.geo.addPoint(0.8, -0.1, 0, size_boundary, 19)
    gmsh.model.geo.addPoint(0.8, 0.1, 0, size_boundary, 20)
    gmsh.model.geo.addPoint(0.2, 0.1, 0, size_boundary, 21)
    gmsh.model.geo.addLine(18, 19, 18)
    gmsh.model.geo.addLine(19, 20, 19)
    gmsh.model.geo.addLine(20, 21, 20)
    gmsh.model.geo.addLine(21, 18, 21)
    gmsh.model.geo.addCurveLoop([18, 19, 20, 21], 3)

    # TOP WINDOW 
    c_x = 0.0
    c_y = 0.5
    R = 0.15
    gmsh.model.geo.addPoint(0, 0.5, 0, size_boundary, 22)
    gmsh.model.geo.addPoint(c_x - R, c_y, 0, size_boundary, 23)
    gmsh.model.geo.addPoint(c_x + R, c_y, 0, size_boundary, 24)

    gmsh.model.geo.addCircleArc(23, 22, 24, 22)
    gmsh.model.geo.addCircleArc(24, 22, 23, 23)

    gmsh.model.geo.addCurveLoop([22, 23], 4)
    ...
    Ldiri = np.arange(1,14)
    gmsh.model.addPhysicalGroup(1, Ldiri, tag_walls)
    gmsh.model.setPhysicalName(1, tag_walls, walls)

    Lneumann = np.arange(14,24)
    gmsh.model.addPhysicalGroup(1, Lneumann, tag_neumann)
...
if __name__ == "__main__" :
    ...
    parser.add_argument("--name",  type=str, default="house_windows", help="Name of the mesh")
