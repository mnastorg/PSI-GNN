...
#########################################################################################################
    ...
    # RECTANGLE
    gmsh.model.geo.addPoint(0, -1, 0, size_boundary, 1)
    gmsh.model.geo.addPoint(1, 0, 0, size_boundary, 2)
    gmsh.model.geo.addPoint(0.75, 1, 0, size_boundary, 3)
    gmsh.model.geo.addPoint(0.5, 0.9, 0, size_boundary, 4)
    gmsh.model.geo.addPoint(0.2, 0.75, 0, size_boundary, 5)
    gmsh.model.geo.addPoint(0, 0.4, 0, size_boundary, 6)
    gmsh.model.geo.addPoint(-0.2, 0.75, 0, size_boundary, 7)
    gmsh.model.geo.addPoint(-0.5, 0.9, 0, size_boundary, 8)
    gmsh.model.geo.addPoint(-0.75, 1, 0, size_boundary, 9)
    gmsh.model.geo.addPoint(-1, 0, 0, size_boundary, 10)

    l1 = [1,2,3,4,5,6]
    l2 = [6,7,8,9,10,1]
    
    gmsh.model.geo.addSpline(l1, 1)
    gmsh.model.geo.addSpline(l2, 2)
    ...
    gmsh.model.addPhysicalGroup(1, [1,2], tag_walls)
...
if __name__ == "__main__" :
    ...
    parser.add_argument("--name",  type=str, default="heart", help="Name of the mesh")
