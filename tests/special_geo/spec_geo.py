...
def test_sample(checkpoint, data, device):
    ...
    table_data = []
    table_data.append([data[1].x.size(0), res_loss_dss, mse_loss_dss, mse_dirichlet_dss])
    ...
    table_data = []
    table_data.append([data[1].x.size(0), res_loss_dsgps, mse_loss_dsgps, mse_dirichlet_dsgps])
    headers = ['Nb nodes','Residual', 'MSE', 'MSEDirichlet']
    print(tabulate(table_data, headers=headers, tablefmt="mixed_grid", floatfmt=".3e"))
    ...
    config_model_psignn["fw_tol"] = 1.e-6
    config_model_psignn["fw_thres"] = 1000
    ...
    table_data = []
    table_data.append([data[0].x.size(0), res_loss_psignn, mse_loss_psignn, mse_dirichlet_psignn, nsteps_psignn])
    headers = ['Nb nodes','Residual', 'MSE', 'MSEDirichlet', 'Nstep']
    print(tabulate(table_data, headers=headers, tablefmt="mixed_grid", floatfmt=".3e"))

    torch.cuda.empty_cache()
    
    # list_sol = [U_sol_dss[str(len(U_sol_dss)-1)], U_sol_dss[str(len(U_sol_dsgps)-1)], U_sol_psignn]
