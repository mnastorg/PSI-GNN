...
class BuildDataset(InMemoryDataset):
    ...
    def processed_dir(self):
        return os.path.join(self.root, 'processed_psignn/')
    ...
    def process(self):
        ...
        self.prb_data_mean = torch.tensor([0.0464, -0.0006], dtype = self.precision)
        self.prb_data_std = torch.tensor([9.6267, 3.2935], dtype = self.precision)

        self.distance_mean = torch.tensor([0.0, 0.0, 0.0655], dtype = self.precision)
        self.distance_std = torch.tensor([0.0507, 0.0507, 0.0293], dtype = self.precision)
        ...
        data_train_, data_test = train_test_split(data_list, test_size = 0.2, shuffle = False)
        data_train, data_val = train_test_split(data_train_, test_size = 0.25, shuffle = False)
