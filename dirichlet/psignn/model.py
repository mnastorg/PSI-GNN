...
class ModelDEQDSS(nn.Module):    
    ...
    def forward(self, batch): 
        ...
        h_final, jacobian_loss = self.deqdss(h_initial, batch)
        ...
        loss_dic["mse_dirichlet"] = mse_dirichlet
    ...
    def inference(self, batch): 
        ...
        out = self.deqdss.inference(h_initial, batch)

        u_final = self.autoencoder.decoder(out["result"])

        return u_final

    def iterative_inference(self, batch): 
        ...
        index_boundary = torch.where(batch.tags==1)[0]
...
class DeepEquilibrium(nn.Module):
    ...
    def forward(self, H_init, batch):
        ...
        # compute forward pass and re-engage autograd
        with torch.no_grad():
        ...
        # if gradients are enabled (i.e training mode)
        if torch.is_grad_enabled(): 
            
            with open(os.path.join(self.path_logs, "forward_iteration.csv"), 'a') as f :
                f.write('\n{} \t {}'.format(out_fw['lowest'], out_fw['nstep']))
                f.close()
            ...
            # compute backward hook
            def backward_hook(grad): 
                if self.hook is not None : 
                    self.hook.remove()
                    torch.cuda.synchronize() 
                out_bw = self.config_deq["solver"](     lambda y : autograd.grad(new_H_star, H_star, y, retain_graph=True)[0] + grad, 
                                                        torch.zeros_like(grad),  
                                                        threshold = self.config_deq["bw_thres"], 
                                                        eps = self.config_deq["bw_tol"]
                                                        )
                with open(os.path.join(self.path_logs, "backward_iteration.csv"), 'a') as f :
                    f.write('\n{} \t {}'.format(out_bw['lowest'], out_bw['nstep']))
                    f.close()

                return out_bw['result']

            self.hook = new_H_star.register_hook(backward_hook)
                    
        # if validation or test mode 
        else :
            ...
            # estimate jacobian loss
            jac_loss = jac_loss_estimate(new_H_star, H_star, vecs = 1)
            
            # compute spectral radius with power iteration
            _, sradius = power_method(new_H_star, H_star, n_iters = 150)
            with open(os.path.join(self.path_logs, "spectral_radius.csv"), 'a') as f :
                    f.write('\n{}'.format(sradius.item()))
                    f.close()

        return new_H_star, jac_loss

    def inference(self, H_init, batch):
...
class Function(nn.Module):
    ...
    def __init__(self, n_layers=None, latent_dim=None, edge_features_dim=None, second_member_dim=None, activation=None):
        ...
        self.alpha = nn.Sequential(nn.Linear(3*latent_dim + second_member_dim, 1),nn.Sigmoid()).apply(initialize_weights_xavier)
    ...
    def forward(self, h, h_initial, batch):
        ...
        for k in range(self.n_layers):
            ...
            mp_to = self.phi_to_list[k](h, batch.edge_index, batch.edge_attr)
            mp_from = self.phi_from_list[k](h, batch.edge_index, batch.edge_attr)
            
            concat = torch.cat([h, mp_to, mp_from, batch.prb_data], dim = 1)
            alpha = self.alpha(concat)
            update = alpha * self.update_list[k](concat)  

            if k == self.n_layers - 1:
                h = self.laynorm(h + update)

            else :
                h = h + update
            
            h[index_dirichlet,:] = h_initial[index_dirichlet,:] 
