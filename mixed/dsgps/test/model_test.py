...
class ModelDSGPS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.z_k = MLPActivation([3*self.config["latent_dim"] + 3, self.config["latent_dim"]], nn.Sigmoid())
        self.r_k = MLPActivation([3*self.config["latent_dim"] + 3, self.config["latent_dim"]], nn.Sigmoid())
        self.correction = MLPActivation([3*self.config["latent_dim"] + 3, self.config["latent_dim"]], nn.Tanh())

        self.phi_neumann = Phi_from([2*self.config["latent_dim"]+ 3, self.config["latent_dim"], self.config["latent_dim"]], nn.ReLU())
        self.update_neumann = MLP([2*self.config["latent_dim"] + 5, self.config["latent_dim"], self.config["latent_dim"]], nn.ReLU()) 
    ...
    def forward(self, batch):
        ...
        # a = -1000
        # b = 1000
        # index_interior = torch.where(batch.tags[:,0]==0)[0]
        ...
        for update in range(self.config["k"]) :
            ...
            mp_neumann = self.phi_neumann(H[str(update)], batch.edge_index, batch.edge_attr)
            ...
            concat_neumann = torch.cat([H[str(update)], mp_neumann, batch.prb_data, batch.unit_normal_vector], dim = 1)
            update_neumann = self.update_neumann(concat_neumann)

            # NEXT LATENT STATE
            H[str(update+1)] = H[str(update)] + interior_next
            H[str(update+1)][index_neumann,:] = update_neumann[index_neumann,:]
            ...
            u_detached = U[str(update+1)].detach()
            h_detached = H[str(update+1)].detach()

            # Encoder loss
            cumul_enc[str(update+1)] = self.mse_loss(self.autoencoder.encoder(u_detached), h_detached)
            
            # Autoencoder loss
            cumul_autoenc[str(update+1)] = self.mse_loss(self.autoencoder.decoder(self.autoencoder.encoder(u_detached).detach()), u_detached)
