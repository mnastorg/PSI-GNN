...
def extract_images_results(example, out, img_saved_path) : 
    ...
    tags[:,2] = 200*tags[:,2]
    tags = np.sum(tags, axis= 1)    
    color_mapping = {1: 0, 100: 0.5, 200: 1}
    colors = [color_mapping[values] for values in tags.flatten()]
    colors_map = ['blue', 'red', 'gold']
    colors_position = [0, 0.5, 1]
    cmap = LinearSegmentedColormap.from_list("custom_cmap", list(zip(colors_position, colors_map)))
    plt.scatter(coordinates[:,0], 
                coordinates[:,1], 
                c = colors, 
    ...
    plt.tick_params(axis='both', 
                    ...
                    labelleft=False)
    plt.axis("off")
    cbar = plt.colorbar(orientation = "horizontal", ticks = [0, 0.5, 1])
    cbar.set_ticklabels(['I', 'D', 'N'])
    ...
    plt.plot(xmse[1:], mse_dirichlet_list[1:], 'k-')
    plt.yscale('log')
